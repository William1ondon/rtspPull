...
void FbmReturnBuffer(Fbm* pFbm, VideoPicture* pVPicture, int bValidPicture)
{
    ...
    {
        //note: vp8 will go here, it has not been resolved
        logv("invalid picture status, bUsedByDecoder=0 when picture buffer is returned.");
        //abort();
        pthread_mutex_unlock(&pFbm->mutex);
        return;
    }
            ...
            int* nAlignValue, int nWidth, int nHeight)
{
    ...
    switch(ePixelFormat)
    {
        ...
        case PIXEL_FORMAT_NV12:
        {
            ...
            {
                pPicture->nBufFd = CdcMemGetFd(pFbm->memops, pPicture->pData0);
            }
