...
typedef struct {
  float
      wheel_axis[4]
                [3]; //分别设置前左轮胎，前右轮胎，后左轮胎，后右轮胎的轴心坐标
  bool roll = false;
  bool swing = false;
  float roll_angle = 0;
  float swing_angle = 0;
  float open_axis
      [6]
      [3]; //分别设置前左车门，前右车门，后左车门，后右车门，引擎盖、后备箱转动的轴心坐标
  float open_angle
      [6]; //分别设置前左车门，前右车门，后左车门，后右车门，引擎盖、后备箱的打开角度
  float wheel_matrix[4][16]; //输出值，存储轮胎的转动摆动的矩阵
  float open_matrix[6][16]; //输出值，存储车门，引擎盖和后备箱的打开的矩阵
...
typedef struct {
  ...
  struct CAR3D {
    ...
    avm_car_state_type_t car_state; //设置车状态
  } car_3d;
  GLuint prog; // shader program
} avm_car_type_t;
...
int type:type=0,car 2d; type=1,car 3d;type=2,blind;
...
void xcb_avmlib_car_Render_3d(avm_car_type_t *avm_car_type,
                              ViewParameter &viewParameter,
                              float fBodyAlpha = 1.0);

int xcb_avmlib_wheelState_update(avm_car_type_t *avm_car_type);

/*@param
avm_car_type_t *avm_car_type
ViewParameter *viewParameter,//指向视角参数
int type//type=0,显示俯视图的车底纹;type=1,显示3d环视车底纹的纹理
注意：在不同视口显示viewParameter指针要指向对应的视角参数
*/
void xcb_avmlib_car_Render_bottom(avm_car_type_t *avm_car_type,
                                  ViewParameter &viewParameter, int type);
