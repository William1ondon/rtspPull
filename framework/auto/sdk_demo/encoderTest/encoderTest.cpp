...
int encodeFile(EncodeParam* param, FILE* inFile, FILE* outFile)
{
    ...
    oneFrameLenAlign = getFrameBitSize(ALIGN(param->srcW,16),ALIGN(param->srcH,16), \
                                  param->pixelFormat);
#ifdef USE_PHY_ADDR
    ret = allocOpen(MEM_TYPE_DMA, pIonMem, NULL);
    if (ret < 0)
    {
    ...
    ret = allocAlloc(MEM_TYPE_DMA, pIonMem, NULL);
    if (ret < 0)
    {
    ...
    while (frameIndex < totalFrameNum)
    {
        ...
        flushCache(MEM_TYPE_DMA, pIonMem, NULL);
#else
        packet.dataLen0 = param->srcW * param->srcH;
...
#ifdef USE_PHY_ADDR
    allocFree(MEM_TYPE_DMA, pIonMem, NULL);
    allocClose(MEM_TYPE_DMA, pIonMem, NULL);
#else
