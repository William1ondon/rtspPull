...
mNeedThumbpic(0), mAudioflag(0),
mAudioHdl(0), mCurRecordLockOps(LockOps_CreateNewAfterClose)
{
	istestrtsp = 0;
	storage_state = 0;
...
int RecordCamera::enc_parm_destory(void)
{
	...
	if(rtsp_enc_baseConfig.memops){
		...
		if (ret < 0) {
			LOGE("enc_parm_destory:allocClose failed");
			return -1;
		}
	}
	return 0;
											...
											int32_t msgType, char *dataPtr, void *user)
{
	...
	/*else {
		...
		//F_LOG
		memcpy(ppkt->buf, (void *)p->addrVirY, ppkt->buflen);
	...
	if (usrDatCb != NULL){
		if(0 == sps_pps_flag){
			...
			usrDatCb(0, (char *) sps_pps_data.pBuffer, sps_pps_data.nLength, mCbUserDat);
		}
...
int RecordCamera::stop()
{
    ...
    {
        LOGW("RecordCamera::stop capture thread has already paused");
        pthread_mutex_unlock(&mRecordFileMutex);
        return NO_ERROR;
    }
...
int RecordCamera::stopRecord()
{
	...
	if (recordStat == RECORD_STATE_PAUSED)
	{
	   LOGW("stopRecord capture thread has already paused");
	   pthread_mutex_unlock(&mRecordFileMutex);
	   return NO_ERROR;
	}
...
void RecordCamera::__timer_cb(sigval_t sig)
{
	...
	if (__this != NULL) {
		...
		if (((__this->mCurRecordPos == 10) && (__this->mNeedNewFile == 1))
			...
			int pp = 0;
			LOGW("record __timer_cb timeout  id=%d pos=%d pp=%p", __this->mCameraId,
...
int RecordCamera::AutAudioMuxInit(CdxMuxerMediaInfoT *pmedia_info)
{
    ...
    pmedia_info->audio.nSampleCntPerFrame = 1024; // 固定的,一帧数据的采样点数目
	...
	do {
		...
		if (as > (afile_blksize + RESERVED_SIZE)) {
			...
			if (strlen(p->cur_filename[rmidx]) != 0) {
				...
				if (afile_blksize <= (oldfileSize - gAFileOccupySizePerMinMb)) {	//若旧文件空间比较大且大小是新的2倍以上，空间不匹配，直接删除
				...
				else if ((afile_blksize <= oldfileSize) && (afile_blksize >= (oldfileSize - gAFileOccupySizePerMinMb))) {	//若旧文件空间比需要创建空间大不到1倍，空间匹配，不删除
			...
			for (i = 0; i < strlen(direntp->d_name); ++i) {
				if (direntp->d_name[i] == '-') {
					...
					if ((idx < CM_MAX_PICTURE_NUM) && (idx >= 0) && (idx <= iMaxPicNum)) {
						...
						char tname[256] = { };
						sprintf(tname, "%s/%s", g_PicPath[l32_CameraId]);
						remove(tname);
					}
				}
			}
...
//create parkmonitor path
	...
	if ((dir_ptr = opendir(camera_path)) == NULL) {
		...
		ALOGW("AW :TotalMB= %lluMB  afile_size = %luMB -max_file_num =%d\n", totMB, afile_size,
			  p->cur_max_filenum);
